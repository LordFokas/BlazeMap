...
public class DebouncingThread {
    ...
    public DebouncingThread(String name, Logger logger) {
        this.logger = logger;
        this.domains = new ArrayList<>();
        this.thread = new Thread(this::loop, name + " Debouncer Thread");
        thread.setDaemon(true);
        thread.start();
        logger.info("Starting {} Debouncer Thread", name);
    }
    ...
    private void work() throws InterruptedException {
        ...
        if(wait == 0) return;
        if(wait < 0) {
            logger.error("DebouncingThread: Attempted to wait for {}ms", wait);
            wait = 100;
        };
    ...
    private void loop() {
        while(true) {
            try{
                ...
                logger.error("Exception in DebouncingThread main loop!");
                e.printStackTrace();
            }
            catch(Throwable t){
                logger.error("Throwable in DebouncingThread main loop! ABORTING.");
                t.printStackTrace();
                return;
            }
        }
